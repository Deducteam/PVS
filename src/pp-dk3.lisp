...
(defun flatten-args (args)
  "Transform a list of pvs-style arguments, where arguments of foo(x,y)(z) is
represented as ((x y) (z)) (i.e. a list of lists) to a list of simple arguments
(t1 z) where t1 is a freshly created tuple."
  (mapcar
   (lambda (x)
     (cond
       ((symbolp x) x)
       ((and (consp x) (consp (cdr x)))
        ...
        ;; REVIEW: use mk-tuple-expr, which doesn't typecheck?
        (make!-tuple-expr x))
       ((and (consp x) (null (cdr x))) (car x))
       (t x)))
   args))
...
as ``f (σcons e1 e2) (σcons g1 g2)''."
  ...
  (if (null (type ex))
      ...
      (let* ((op (operator* ex))
             (args (flatten-args (arguments* ex))))
        (with-parens (stream colon-p)
          (pp-dk stream op)
          (write-char #\space stream)
