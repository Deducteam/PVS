...
(defun tuplify-arg (arg)
  "Transform an argument that is either an atom or a list of arguments into a
single argument: lists of arguments are transformed into tuples."
  (cond
    ((atom arg) arg)
    ((and (consp arg) (consp (cdr arg))) (make!-tuple-expr arg))
    ((single arg) (car arg))
...
as ``f (σcons e1 e2) (σcons g1 g2)''."
  ...
  (if (null (type ex))
      ...
      (let* ((op (operator* ex))
             (args (mapcar #'tuplify-arg (arguments* ex))))
        (with-parens (stream colon-p)
          (pp-dk stream op)
          (write-char #\space stream)
